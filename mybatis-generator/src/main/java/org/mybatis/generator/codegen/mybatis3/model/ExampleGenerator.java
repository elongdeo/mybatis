...
public class ExampleGenerator extends AbstractJavaGenerator {
    ...
    private InnerClass getGeneratedCriteriaInnerClass(
        ...
        Method method = new Method();
        method.setVisibility(JavaVisibility.PROTECTED);
        method.setName("AbstractCriteria");
        method.setConstructor(true);
        method.addBodyLine("super();");
        ...
        method = new Method();
        method.setVisibility(JavaVisibility.PUBLIC);
        method.setName("addCustomizedCriterion");
        method.addParameter(new Parameter(FullyQualifiedJavaType
                .getStringInstance(), "condition"));
        method.addBodyLine("if (condition == null) {");
        method.addBodyLine("throw new IllegalArgumentException(\"Value for condition cannot be null\");");
        method.addBodyLine("}");
        ...
        method.addBodyLine(CommonPluginUtil.getIgnoredCriterionValueNullResult(introspectedTable,
        ...
        if (introspectedTable.hasJDBCTimeColumns()) {
            ...
            method
                .addBodyLine(
                    "addCriterion(condition, new java.sql.Time(value1.getTime()), new java.sql.Time(value2.getTime())"
                            + ", property);");
            answer.addMethod(method);
        }
        ...
        for (IntrospectedColumn introspectedColumn : introspectedTable
            ...
            if (introspectedColumn.getFullyQualifiedJavaType().getFullyQualifiedName().equals(String.class.getName())) {
                answer.addMethod(getSetRegexpMethod(introspectedColumn));
            }
        }
    ...
    private Method getSetNullMethod(IntrospectedColumn introspectedColumn) {
        return getNoValueBlankMethod(introspectedColumn, "IsNull", "isnull");
    }

    private Method getSetNotNullMethod(IntrospectedColumn introspectedColumn) {
        return getNoValueBlankMethod(introspectedColumn, "IsNotNull", "not isnull");
    }
    ...
    private Method getSetRegexpMethod(IntrospectedColumn introspectedColumn) {
        return getSingleValueMethod(introspectedColumn, "Regexp", "regexp");
    }
    ...
    private Method getNoValueBlankMethod(IntrospectedColumn introspectedColumn,
                                         String nameFragment, String operator) {
        Method method = new Method();
        method.setVisibility(JavaVisibility.PUBLIC);
        StringBuilder sb = new StringBuilder();
    ...
    /**
     * This method adds all the extra methods and fields required to support a user defined type handler on some
     * column.
     *
     * @param introspectedColumn the introspected column
    ...
    private void checkValue(Method method) {
        method.addBodyLine("if (value == null) {");
        method.addBodyLine(CommonPluginUtil.getIgnoredCriterionValueNullResult(introspectedTable,
                "throw new IllegalArgumentException(\"Value for \" + property + \" cannot be null\");"));
        method.addBodyLine("} else if (value instanceof List && ((List) value).isEmpty()) {");
        method.addBodyLine(CommonPluginUtil.getIgnoredCriterionValueNullResult(introspectedTable,
                "throw new IllegalArgumentException(\"Value for \" + property + \" cannot be empty\");"));
        method.addBodyLine("}");
    }
